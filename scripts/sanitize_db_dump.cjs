...
`;
  ...
  // 3. Add IF NOT EXISTS / OR REPLACE to create statements for idempotency
  
  // Schemas
  sql = sql.replace(/CREATE SCHEMA /g, 'CREATE SCHEMA IF NOT EXISTS ');

  // Tables
  sql = sql.replace(/CREATE TABLE /g, 'CREATE TABLE IF NOT EXISTS ');
  
  // Sequences
  sql = sql.replace(/CREATE SEQUENCE /g, 'CREATE SEQUENCE IF NOT EXISTS ');
  
  // Indexes (Handle UNIQUE first to avoid double replacement if regex was loose, though string literal is fine)
  ...
  // Triggers (Postgres 14+)
  sql = sql.replace(/CREATE TRIGGER /g, 'CREATE OR REPLACE TRIGGER ');
  
  // Extensions (Normalize first to avoid double IF NOT EXISTS)
  sql = sql.replace(/CREATE EXTENSION IF NOT EXISTS /g, 'CREATE EXTENSION ');
  sql = sql.replace(/CREATE EXTENSION /g, 'CREATE EXTENSION IF NOT EXISTS ');

  // 4. Remove or comment out specific Supabase extensions that might not be available or needed locally
