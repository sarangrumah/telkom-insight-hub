...
export function useTelekomDataList(params: TelekomListParams = {}, enabled = true) {
  return useQuery<ListResponse, Error>({
    ...
    queryFn: async () => {
      ...
      const q = qs.toString();
      const url = q ? `/v2/panel/api/telekom-data?${q}` : '/v2/panel/api/telekom-data';
...
export function useCreateTelekomData() {
  ...
  return useMutation({
    mutationFn: async (payload: CreatePayload) => {
      const data = await apiFetch('/v2/panel/api/telekom-data', {
        method: 'POST',
...
export function useUpdateTelekomData() {
  ...
  return useMutation({
    mutationFn: async (payload: UpdatePayload) => {
      ...
      const data = await apiFetch(`/v2/panel/api/telekom-data/${id}`, {
        method: 'PATCH',
