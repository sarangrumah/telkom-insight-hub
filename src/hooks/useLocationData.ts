...
export const useLocationData = () => {
  ...
  useEffect(() => {
    const fetchLocationData = async () => {
      try {
        ...
        const provincesResp = await apiFetch('/v2/panel/api/provinces') as ProvincesResponse;
        const kabupatenResp = await apiFetch('/v2/panel/api/kabupaten') as KabupatenResponse;
  ...
  const fetchKecamatanByKabupaten = async (kabupatenId: string): Promise<Kecamatan[]> => {
    try {
      const response = await apiFetch(`/v2/panel/api/kecamatan?kabupaten_id=${kabupatenId}`) as KecamatanResponse;
  ...
  const fetchKelurahanByKecamatan = async (kecamatanId: string): Promise<Kelurahan[]> => {
    try {
      const response = await apiFetch(`/v2/panel/api/kelurahan?kecamatan_id=${kecamatanId}`) as KelurahanResponse;
