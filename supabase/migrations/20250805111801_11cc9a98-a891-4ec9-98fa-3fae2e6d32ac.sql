...
-- This maps the current service_type enum and sub_service_type text to the new structure
...
WHERE s.code = CASE 
  WHEN td.service_type = 'JASA' THEN 'JASA'
  WHEN td.service_type = 'JARINGAN' THEN 'JARINGAN'
  WHEN td.service_type = 'TELEKOMUNIKASI_KHUSUS' THEN 'TELEKOMUNIKASI_KHUSUS'
END
AND ss.name = COALESCE(td.sub_service_type, 
  CASE 
    WHEN td.service_type = 'JASA' THEN 'Jasa Akses Internet'
    WHEN td.service_type = 'JARINGAN' THEN 'Penyelenggaraan Jaringan Tetap Lokal'
    WHEN td.service_type = 'TELEKOMUNIKASI_KHUSUS' THEN 'Very Small Aperture Terminal (VSAT)'
  END
);
