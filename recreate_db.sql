...
CREATE OR REPLACE FUNCTION public.approve_company(_company_id uuid, _verified_by uuid, _notes text DEFAULT NULL::text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.calculate_sla_metrics() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.check_record_permission(_user_id uuid, _table_name text, _record_id uuid, _action text) RETURNS boolean
...
CREATE OR REPLACE FUNCTION public.check_user_permission(_user_id uuid, _module_code text, _action text, _field_code text DEFAULT NULL::text) RETURNS boolean
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.escalate_overdue_tickets() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_user_permissions(_user_id uuid, _module_code text DEFAULT NULL::text) RETURNS TABLE(module_code text, module_name text, field_code text, field_name text, can_create boolean, can_read boolean, can_update boolean, can_delete boolean, field_access text)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.reject_company(_company_id uuid, _rejected_by uuid, _rejection_notes text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.request_company_correction(_company_id uuid, _requested_by uuid, _correction_notes jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.submit_company_corrections(_company_id uuid, _submitted_by uuid, _updated_data jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.user_has_role(_user_id uuid, _role public.user_role) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    AS $$
  SELECT EXISTS (
    SELECT 1
...
CREATE TABLE IF NOT EXISTS public.indonesian_regions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.license_services (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    name text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.telekom_data (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.ticket_sla_metrics (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    ticket_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.ulo_applications (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE realtime.messages_2025_09_11 OWNER TO postgres;
...
ALTER TABLE realtime.messages_2025_09_12 OWNER TO postgres;
...
ALTER TABLE realtime.messages_2025_09_13 OWNER TO postgres;
...
ALTER TABLE realtime.messages_2025_09_14 OWNER TO postgres;
...
ALTER TABLE realtime.messages_2025_09_15 OWNER TO postgres;
...
ALTER TABLE realtime.messages_2025_09_16 OWNER TO postgres;
...
CREATE TABLE IF NOT EXISTS realtime.messages_2025_09_17 (
    topic text NOT NULL,
    extension text NOT NULL,
    payload jsonb,
    event text,
...
ALTER TABLE realtime.messages_2025_09_17 OWNER TO postgres;
...
CREATE INDEX IF NOT EXISTS oauth_clients_client_id_idx ON auth.oauth_clients USING btree (client_id);
...
CREATE INDEX IF NOT EXISTS idx_api_integration_logs_api_name ON public.api_integration_logs USING btree (api_name);
...
CREATE INDEX IF NOT EXISTS idx_api_integration_logs_created_at ON public.api_integration_logs USING btree (created_at);
...
CREATE INDEX IF NOT EXISTS idx_api_integration_logs_status ON public.api_integration_logs USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_api_integration_logs_user_id ON public.api_integration_logs USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_companies_nib_number ON public.companies USING btree (nib_number);
...
CREATE INDEX IF NOT EXISTS idx_companies_npwp_number ON public.companies USING btree (npwp_number);
...
CREATE INDEX IF NOT EXISTS idx_company_documents_company_id ON public.company_documents USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_kabupaten_province_id ON public.kabupaten USING btree (province_id);
...
CREATE INDEX IF NOT EXISTS idx_person_in_charge_company_id ON public.person_in_charge USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_pic_documents_pic_id ON public.pic_documents USING btree (pic_id);
...
CREATE INDEX IF NOT EXISTS idx_regions_name ON public.indonesian_regions USING btree (name);
...
CREATE INDEX IF NOT EXISTS idx_regions_parent_id ON public.indonesian_regions USING btree (parent_id);
...
CREATE INDEX IF NOT EXISTS idx_regions_type ON public.indonesian_regions USING btree (type);
...
CREATE INDEX IF NOT EXISTS idx_telekom_data_kabupaten_id ON public.telekom_data USING btree (kabupaten_id);
...
CREATE INDEX IF NOT EXISTS idx_telekom_data_province_id ON public.telekom_data USING btree (province_id);
...
CREATE INDEX IF NOT EXISTS idx_telekom_data_sub_service_id ON public.telekom_data USING btree (sub_service_id);
...
CREATE INDEX IF NOT EXISTS idx_telekom_data_sub_service_type ON public.telekom_data USING btree (sub_service_type);
...
CREATE INDEX IF NOT EXISTS idx_ticket_messages_created_at ON public.ticket_messages USING btree (created_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS subscription_subscription_id_entity_filters_key ON realtime.subscription USING btree (subscription_id, entity, filters);
...
CREATE OR REPLACE TRIGGER api_integration_logs_audit_trigger AFTER INSERT OR DELETE OR UPDATE ON public.api_integration_logs FOR EACH ROW EXECUTE FUNCTION public.log_changes();
...
CREATE OR REPLACE TRIGGER audit_telekom_data_changes AFTER INSERT OR DELETE OR UPDATE ON public.telekom_data FOR EACH ROW EXECUTE FUNCTION public.log_changes();
...
CREATE OR REPLACE TRIGGER audit_tickets_changes AFTER INSERT OR DELETE OR UPDATE ON public.tickets FOR EACH ROW EXECUTE FUNCTION public.log_changes();
...
CREATE OR REPLACE TRIGGER audit_user_roles_changes AFTER INSERT OR DELETE OR UPDATE ON public.user_roles FOR EACH ROW EXECUTE FUNCTION public.log_changes();
...
CREATE OR REPLACE TRIGGER update_api_integration_logs_updated_at BEFORE UPDATE ON public.api_integration_logs FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_fields_updated_at BEFORE UPDATE ON public.fields FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_kabupaten_updated_at BEFORE UPDATE ON public.kabupaten FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_license_applications_updated_at BEFORE UPDATE ON public.license_applications FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_license_services_updated_at BEFORE UPDATE ON public.license_services FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_person_in_charge_updated_at BEFORE UPDATE ON public.person_in_charge FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_pic_documents_updated_at BEFORE UPDATE ON public.pic_documents FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_sla_metrics_on_message AFTER INSERT ON public.ticket_messages FOR EACH ROW EXECUTE FUNCTION public.calculate_sla_metrics();
...
CREATE OR REPLACE TRIGGER update_sla_metrics_on_status_change AFTER UPDATE OF status ON public.tickets FOR EACH ROW EXECUTE FUNCTION public.calculate_sla_metrics();
...
CREATE OR REPLACE TRIGGER update_sub_services_updated_at BEFORE UPDATE ON public.sub_services FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_telekom_data_updated_at BEFORE UPDATE ON public.telekom_data FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_ticket_messages_updated_at BEFORE UPDATE ON public.ticket_messages FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_ulo_applications_updated_at BEFORE UPDATE ON public.ulo_applications FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
